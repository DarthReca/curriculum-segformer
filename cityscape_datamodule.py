...
class CityscapesDataModule(LightningDataModule):
    ...
    def __init__(
        ...
        sampling_milestones: Optional[List[int]] = None,
        *args: Any,
        **kwargs: Any,
    ) -> None:
        """
        ...
        # MODIFICATION
        self.sampling_milestones = (
            np.array(sampling_milestones) - 1 if sampling_milestones else None
        )
        # END MODIFICATION
    ...
    def train_dataloader(self) -> DataLoader:
        ...
        # MODIFICATION
        if self.sampling_milestones is not None:
            pct = (self.sampling_milestones <= self.current_epoch).sum() / len(
                self.sampling_milestones
            )
            samples = int(pct * len(dataset))
            # if sample is not multiple of batch size and pct is not 1, round down
            samples = samples - (samples % self.batch_size) if pct < 1 else samples
            # Make a subset of the dataset with the first pct of the data
            dataset = Subset(dataset, np.arange(samples))
            logging.info(f"Using only {pct:.1%} of the training data")
        # END MODIFICATION
